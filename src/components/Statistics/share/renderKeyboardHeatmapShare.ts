...
export async function renderKeyboardHeatmapSharePng({
  ...
  drawKeyboardSection({
    title: strings.unshiftedSectionTitle ?? 'Unshifted',
    counts: unshiftedCounts,
    showShiftedLabel: false,
  })
  cursorY += sectionGap
  drawKeyboardSection({
    title: strings.shiftedSectionTitle ?? 'Shifted',
  ...
  ctx.textBaseline = 'middle'
  ctx.fillText(strings.legendLow ?? 'Low', contentX, cursorY + legendH / 2)
  ...
  ctx.fillStyle = 'rgba(15,23,42,0.55)'
  ctx.font = '600 12px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial'
  ctx.fillText(strings.legendHigh ?? 'High', contentX + contentW, cursorY + legendH / 2)
  ctx.textAlign = 'left'
