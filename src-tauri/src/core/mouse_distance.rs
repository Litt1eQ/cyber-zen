...
const MONITOR_REFRESH_INTERVAL: Duration = Duration::from_secs(6);
...
struct TrackingState {
    cursor: CursorState,
    pending_distance_mp_by_display: HashMap<String, u64>,
}
...
pub fn init(app_handle: AppHandle) {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            let (stats, carry) = {
                let mut carry: HashMap<String, u64> = HashMap::new();
                let storage = MeritStorage::instance();
...
pub fn record_mouse_move(space: CoordinateSpace, x: f64, y: f64) {
    ...
    let (display_id, px, py) = match space {
        CoordinateSpace::Physical => {
            let id = display_id_for_point(CoordinateSpace::Physical, x, y);
            (id, x, y)
        }
        CoordinateSpace::Logical => match logical_to_physical_with_display_id(x, y) {
            Some((id, p)) => (Some(id), p.0, p.1),
            None => (None, x, y),
        },
    };
            ...
            if dx.abs() <= MAX_JUMP_PX && dy.abs() <= MAX_JUMP_PX {
                ...
                if dist.is_finite() && dist > 0.0 {
                    ...
                    if mp.is_finite() && mp > 0.0 {
                        let key = display_id.unwrap_or_else(|| "unknown".to_string());
                        st.pending_distance_mp_by_display
                            .entry(key)
...
fn logical_to_physical_with_display_id(x: f64, y: f64) -> Option<(String, (f64, f64))> {
    let monitors = MONITORS.read();
    let monitor = monitors.iter().find(|m| m.contains(CoordinateSpace::Logical, x, y));
    monitor.and_then(|m| m.physical_from_logical(x, y).map(|p| (m.id.clone(), p)))
}

fn display_id_for_point(space: CoordinateSpace, x: f64, y: f64) -> Option<String> {
    let monitors = MONITORS.read();
    let monitor = monitors.iter().find(|m| m.contains(space, x, y));
    monitor.map(|m| m.id.clone())
}
...
fn refresh_monitors(app_handle: &AppHandle) {
    ...
    for m in monitors {
        ...
        out.push(MonitorSnapshot {
            ...
            logical_right,
            logical_bottom,
            id: crate::core::click_heatmap::monitor_id(&m),
        });
    }
